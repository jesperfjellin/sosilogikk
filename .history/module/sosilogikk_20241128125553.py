    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as file:
            ...
            for line_number, line in enumerate(file, 1):
                ...
                if in_header:
                    logger.debug(f"Processing header line: {stripped_line}")
                    ...
                    if stripped_line.startswith('...'):
                        logger.debug(f"Processing three-dot attribute in section {current_section}")
                ...
                if in_header and stripped_line.startswith('.') and not stripped_line.startswith('..'):
                    if stripped_line != '.HODE':
                        logger.debug(f"Exiting header section at line: {stripped_line}")
                        in_header = False
                        current_section = None
                        parent_section = None
        ...
        # Before raising the error, log the final state
        logger.debug(f"Final enhet_scale value: {enhet_scale}")
        logger.debug(f"Final header_metadata: {header_metadata}")
