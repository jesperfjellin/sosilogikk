        ...
        with open(filepath, 'r', encoding='utf-8') as file:
            ...
            for line_number, line in enumerate(file, 1):
                ...
                if in_header:
                    ...
                    # Handle three-dot attributes (like ...ENHET under ..TRANSPAR)
                    if stripped_line.startswith('...'):
                        attr_name = stripped_line[3:].split(maxsplit=1)[0]
                        
                        # Only process if we have a value after the attribute name
                        if len(stripped_line[3:].split(maxsplit=1)) > 1:
                            attr_value = stripped_line[3:].split(maxsplit=1)[1]
                            
                            # Handle specific attributes based on their parent section
                            if current_section == 'TRANSPAR':
                                if attr_name == 'ENHET':
                                    try:
                                        enhet_scale = float(attr_value)
                                        header_metadata['ENHET'] = enhet_scale
                                        logger.debug(f"Found ENHET value: {enhet_scale} under TRANSPAR")
                            ...
                            # Handle OMRÅDE section
                            elif current_section == 'OMRÅDE':
                                if attr_name == 'MIN-NØ':
                                    try:
                                        parts = stripped_line.split()
                                        min_n, min_e = float(parts[-2]), float(parts[-1])
                                    except (ValueError, IndexError) as e:
                                        logger.error(f"Error parsing MIN-NØ: {e}")
                                        raise
                                elif attr_name == 'MAX-NØ':
                                    try:
                                        parts = stripped_line.split()
                                        max_n, max_e = float(parts[-2]), float(parts[-1])
                                    except (ValueError, IndexError) as e:
                                        logger.error(f"Error parsing MAX-NØ: {e}")
                                        raise

                    # Handle two-dot attributes in HODE
                    elif stripped_line.startswith('..'):
                        attr_name = stripped_line[2:].split(maxsplit=1)[0]
                        if len(stripped_line[2:].split(maxsplit=1)) > 1:
                            attr_value = stripped_line[2:].split(maxsplit=1)[1]
                            if attr_name == 'SOSI-VERSJON':
                                header_metadata['SOSI-VERSJON'] = attr_value
                            elif attr_name == 'SOSI-NIVÅ':
                                header_metadata['SOSI-NIVÅ'] = attr_value
                            elif attr_name == 'OBJEKTKATALOG':
                                header_metadata['OBJEKTKATALOG'] = attr_value
