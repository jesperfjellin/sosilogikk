...
def write_geodataframe_to_sosi(gdf, output_file, metadata=None):
    """
    ...
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            if 'VERT-DATUM' in metadata:
                f.write(f'...VERT-DATUM {metadata["VERT-DATUM"]}\n')
            if 'KOORDSYS' in metadata:
                f.write(f'...KOORDSYS {metadata["KOORDSYS"]}\n')
            f.write('...ORIGO-NØ 0 0\n')
            
            # Write OMRÅDE section
            f.write('..OMRÅDE\n')
            f.write(f'...MIN-NØ {bounds_min[1]:.2f} {bounds_min[0]:.2f}\n')
            f.write(f'...MAX-NØ {bounds_max[1]:.2f} {bounds_max[0]:.2f}\n')
            
            # Write version info
            if 'SOSI-VERSJON' in metadata:
                f.write(f'..SOSI-VERSJON {metadata["SOSI-VERSJON"]}\n')
            if 'SOSI-NIVÅ' in metadata:
                f.write(f'..SOSI-NIVÅ {metadata["SOSI-NIVÅ"]}\n')
            if 'OBJEKTKATALOG' in metadata:
