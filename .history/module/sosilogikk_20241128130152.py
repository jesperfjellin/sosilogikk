    ...
    try:
        with open(filepath, 'r', encoding='utf-8-sig') as file:
            ...
            for line_number, line in enumerate(file, 1):
                ...
                # Exit header section and process geometry/attributes
                elif stripped_line.startswith('.'):
                    if capturing:
                        # Save the current object before starting a new one
                        if current_object:
                    ...
                    if stripped_line.startswith('.KURVE'):
                        geom_type = '.KURVE'
                        coordinates = []
                        expecting_coordinates = False
                    elif stripped_line.startswith('.FLATE'):
                        geom_type = '.FLATE'
                        coordinates = []
                        flate_refs = []
                        expecting_coordinates = False
                    elif stripped_line.startswith('.PUNKT'):
                        geom_type = '.PUNKT'
                        coordinates = []
                        expecting_coordinates = False
                    ...
                    current_object.append(stripped_line)
                    if stripped_line.startswith('..OBJTYPE'):
                        current_attributes['OBJTYPE'] = stripped_line.split()[-1]
                        all_attributes.add('OBJTYPE')
                    elif stripped_line.startswith('..NØH'):
                        coordinate_dim = 3
                        expecting_coordinates = True
                    elif stripped_line.startswith('..NØ'):
                        coordinate_dim = 2
                        expecting_coordinates = True
