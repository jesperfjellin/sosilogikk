...
def analyze_gdf(gdf):
    ...
    # Get the total length of all geometries
    total_length = gdf.geometry.length.sum()
    logger.info(f"Total length of geometries: {total_length:.2f} meters")
    
    # Count unique values in each column (excluding geometry)
    for column in gdf.columns:
        if column != 'geometry':
            unique_count = gdf[column].nunique()
            logger.info(f"Number of unique values in {column}: {unique_count}")
...
def main():
    ...
    input_dir = Path(r'C:\FKKOslo\Kartkontroll\2023\Viken_FKB-C_del_23\Elveg')
    output_dir = Path(r'C:\FKKOslo\Kartkontroll\2023\Viken_FKB-C_del_23\Elveg\Output')
