...
def analyze_gdf(gdf):
    ...
    """
    logger.info(f"Antall objekter: {len(gdf)}")
    
    # Enkel analyse: Beregn grunnleggende statistikk
    logger.info("Utfører grunnleggende analyse...")
    
    # Beregn total lengde av alle geometrier
    total_length = gdf.geometry.length.sum()
    logger.info(f"Total lengde av geometrier: {total_length:.2f} meter")
    
    # Tell unike verdier i hver kolonne (unntatt geometri)
    for column in gdf.columns:
        if column != 'geometry':
            unique_count = gdf[column].nunique()
            logger.info(f"Antall unike verdier i {column}: {unique_count}")
